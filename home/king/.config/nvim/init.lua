...
vim.opt.list = true  -- 启用特殊字符显示
vim.opt.listchars = {
  tab = "⟩‐",        -- tab 键 '—'，必须两个字符，例如 “->” 其中 - 为第一个显示的字符，> 为循环显示的字符，若 tab 宽度为 4 ，则显示为 ->>>
  trail = "␣",       -- 末尾空格
  extends = "⟩",     -- 文字超出窗口右侧提示符
  precedes = "⟨",    -- 文字超出窗口左侧提示符
}
...
vim.opt.softtabstop    = 4              -- 退格键删除 4 个空格的宽度
-- makefile 文件不替换 tab 为空格
